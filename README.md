...
- External integrations: public holiday lookup, weather information, and a pluggable notification client.
...
- **External services**: Weather and holiday lookups via `WebClient` adapters treat providers as black boxes; failures are handled gracefully. A notification port/adaptor can call an external notification service when enabled.
...
- **Weather API**: `OpenMeteoWeatherAdapter` (via `WeatherService`) fetches simple weather data for display.
